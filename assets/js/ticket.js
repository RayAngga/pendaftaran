...
async function renderTicketCard(rec){
  ...
  if (!el("t-download")) {
    ...
    btnBar.className = "mt-4 flex justify-center";
    btnBar.innerHTML = `
  ...
  if (!cv) { cv = document.createElement("canvas"); cv.id = "ticket-canvas"; wrap.innerHTML = ""; wrap.appendChild(cv); }
...
export async function findTicket(){
  ...
  try {
    ...
    let { rec } = await api.getTicket(raw);

  
    if (!rec && /^\+?\d[\d\s-]{6,}$/.test(raw)) {
      const f = await api.findByWA(raw);
      rec = f?.rec;
    }
...
function bindDownloadButton(){
  el("t-download")?.addEventListener("click", () => {
    ...
    a.href = cv.toDataURL("image/png"); a.download = `${code}.png`;
