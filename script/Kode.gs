...
function getSheet() {
  ...
  // jika header beda → tulis ulang header SAJA (jangan clear data)
...
function toObj(row) {
  ...
  o.attended = Number(o.attended || 0);
  return o;
}

function toRow(o) {
  // Pastikan tepat = HEADERS.length (11)
  const dict = {
    id: o.id || '',
    createdAt: o.createdAt || nowStr(),
    nama: String(o.nama||'').trim(),
    fakultas: String(o.fakultas||'').trim(),
    prodi: String(o.prodi||'').trim(),
    wa: normWa(o.wa || ''),
    domisili: String(o.domisili||'').trim(),
    paid: Number(o.paid || 0),
    attended: Number(o.attended || 0),
    code: String(o.code || ''),
    proofUrl: String(o.proofUrl || '')
  };
  return HEADERS.map(h => (h in dict ? dict[h] : ''));
}
...
function readAll() {
  ...
  const last = sh.getLastRow();
  if (last < 2) return [];
  const rows = sh.getRange(2,1,last-1,HEADERS.length).getValues().map(toObj);
  return rows.map((r,i)=> ({ ...r, _row: i+2 }));
}

function writeRow(rowIndex, obj) {
  const sh = getSheet();
  sh.getRange(rowIndex,1,1,HEADERS.length).setValues([toRow(obj)]);
}
...
/** ====== ACTIONS ====== */
function actionList() {
  return { ok:true, rows: readAll() };
}

function actionRegister(p) {
  if (!p || !p.nama || !p.wa)
    return { ok:false, error:'nama & wa wajib' };

  const lock = LockService.getScriptLock(); lock.waitLock(5000);
  try {
    const rows = readAll();
    const waNorm = normWa(p.wa);
    ...
    const rec = {
      ...
      wa: waNorm,
      domisili: String(p.domisili||'').trim(),
      paid: 0,
      attended: 0,
      code: '',
      proofUrl: ''
    };
    sh.appendRow(toRow(rec));
...
function actionFindByWA(p) {
  const wa = normWa((p && p.wa) || '');
  const rows = readAll();
  const rec = rows.find(r => normWa(r.wa) === wa);
...
function actionGetTicket(p) {
  ...
  if (/^GTFLOW-/i.test(q)) {
    ...
    rec = rows.find(r => String(r.code||'').toUpperCase() === qc);
  } else {
    const wa = normWa(q);
    rec = rows.find(r => normWa(r.wa) === wa) ||
          rows.find(r => String(r.code||'').toUpperCase() === q.toUpperCase());
  }
  return { ok:true, rec: rec || null };
}
...
function actionSubmitProof(p) {
  ...
  const pidNorm = normWa(pid);
  const pidCode = pid.toUpperCase();

  let rec = rows.find(r =>
    r.id === pid ||
    String(r.code||'').toUpperCase() === pidCode ||
    normWa(r.wa) === pidNorm
  );
  if (!rec) return { ok:false, error:'record not found' };

  const fileNameBase = (rec.nama || 'bukti') + '_' + Date.now();
  const proofUrl = saveDataUrlToDrive(p.dataUrl, fileNameBase);
  if (!rec.code) rec.code = genCode();
  rec.paid = 1;
  rec.proofUrl = proofUrl;
  writeRow(rec._row, rec);
  return { ok:true, id: rec.id, code: rec.code, proofUrl };
}

function actionToggle(p, field) {
  if (!p || !p.id) return { ok:false, error:'missing id' };
  const rows = readAll();
  const rec = rows.find(r => r.id === p.id);
  if (!rec) return { ok:false, error:'not found' };
  rec[field] = Number(rec[field]) ? 0 : 1;
  writeRow(rec._row, rec);
  return { ok:true, id: rec.id, [field]: rec[field] };
}
...
function genCode() {
  ...
  do {
    ...
    code = `GTFLOW-${p}${t}`;
  } while (rows.some(r => String(r.code||'') === code));
  return code;
}

function saveDataUrlToDrive(dataUrl, fileNameBase) {
  ...
  const folder = getOrCreateFolder(PROOF_FOLDER);
