...
async function parseBody(req){
  if (req.body) return req.body; // kalau sudah ada (Vercel kadang sudah parse)
...
export default async function handler(req, res){
  ...
  try{
    ...
    // ==== WHITELIST: aksi publik tidak perlu admin ====
    ...
    const r = await fetch(url, {
      method: "POST",
      // cache no-store supaya selalu kena Apps Script versi terbaru
      cache: "no-store",
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({ ...(body||{}), secret })
    });

    // Robust parse: jika bukan JSON, kembalikan cuplikan teksnya agar error jelas
    ...
    try {
      ...
      return res.status(r.status || 500).json({
        ok: false,
        error: `Apps Script returned non-JSON (${r.status}): ${raw.slice(0,200)}`
      });
    }
