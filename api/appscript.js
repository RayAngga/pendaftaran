...
export default async function handler(req, res){
  ...
  if (req.method === "OPTIONS") {
    res.setHeader("Access-Control-Allow-Origin", allowedOrigin || "*"); res.setHeader('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate');
  ...
  try{
    ...
    if(!url || !secret) return res.status(500).json({ ok:false, error:"APPSSCRIPT_URL/SECRET not set" });
    ...
    res.setHeader('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate'); return res.status(500).json({ ok:false, error:String(e) });
  }
}
